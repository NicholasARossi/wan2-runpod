"""
Provider-agnostic core logic for Wan 2.2 Image-to-Video handler.
================================================================
Extracted from handler.py so both RunPod and Vast.ai wrappers can
share the same pipeline code.
...
def process_request(job_input: dict) -> dict:
    """Run the full I2V pipeline. Provider-agnostic entry point.

    Takes the job input dict directly (not the RunPod job wrapper).
    Returns dict with either {video: base64_string} or {error: message}.
    """
    task_id = f"task_{uuid.uuid4()}"
    logger.info("Received request, task_id=%s", task_id)

    # Health check: return immediately without GPU work
    if job_input.get("health_check"):
        logger.info("Health check passed")
        return {"status": "ok", "message": "wan2-runpod is running"}
    ...
    # High/Low noise split point (50% of steps for high noise)
