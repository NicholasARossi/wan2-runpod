# wan2-runpod: Wan 2.2 I2V RunPod Serverless Endpoint (Ada variant)
# SageAttention compiled for sm_89 (Ada Lovelace) — targets L40S / RTX 6000 Ada (48GB).
# Same structure as Blackwell base (includes CUDA dev tools, PyTorch, RunPod infra).
# The runtime-only nvidia base was causing worker crashes because ComfyUI custom nodes
# need CUDA compilation at startup.
...
# Download ALL models in parallel (~36GB baked into image to avoid NV dependency)
# Base diffusion models (2x ~7.5GB fp8), text encoder (~6.7GB), VAE (~1GB),
# clip_vision (~1.2GB), upscaler (~64MB), 2x Lightning LoRAs (~1.3GB)
...
# Verify critical model downloads (including base diffusion models)
